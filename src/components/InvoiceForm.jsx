...
import { addInvoice, updateInvoice } from "../redux/invoice-slice";
import { Link, useParams, useLocation, useNavigate } from "react-router-dom";
import generateRandomId from "../helper";
import { useInvoices } from "../hooks/useInvoices";

const InvoiceForm = ({ isCopy, isEdit, invoiceId }) => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { getOneInvoice, listSize } = useInvoices();
  ...
  const handleAddInvoice = () => {
    if (isEdit) {
      dispatch(updateInvoice({ id: invoiceId, updatedInvoice: formData }));
  ...
  return (
    <Form onSubmit={openModal} className="p-4">
      <div className="d-flex align-items-center">
        <BiChevronLeft size={18} />
        <div className="fw-bold mt-2 mx-2 cursor-pointer">
